...
// SetUartRemote.......................................................................................
//   New: Thunk 
//     Use Thunk and switch addressing
//
//   Old:  UART redirect register
...
//   Depends on var bCnxToCu to be set at startup and does not ever change.
    ...
    if( bUseThunkLayer )
    {
        ...
        else
        {
            if( bRxThunkPending == false )
            {
                if( bCnxToCu )
                {
                    // Initially connected to CU so switch addressing to NU
                    PrintLog(1,  "SetUartRemote(NU): Thunk:  Call SwitchCelFiAddress(" + CELFI_ADDRESS_ID_NU + ")" );
                    Module._SwitchCelFiAddress(CELFI_ADDRESS_ID_NU);
                }
                else
                {
                    // Initially connected to NU so switch addressing to CU
                    PrintLog(1,  "SetUartRemote(CU): Thunk:  Call SwitchCelFiAddress(" + CELFI_ADDRESS_ID_CU1 + ")" );
                    Module._SwitchCelFiAddress(CELFI_ADDRESS_ID_CU1);
                }
                
                // Now get the new board config to verify that we are remote.
                GetBoardConfig();
            }
            else
            {
                PrintLog(1,  "SetUartRemote(): Thunk called but thunk pending so do nothing" );
            }
...
function SetUartLocal()
{
    ...
    if( bCnxToOneBoxNu )
    {
        PrintLog(1,  "SetUartLocal(): Super Msg Send: Cancel UART Redirect (Not required on 1-Box)" );
    }
    else
    {
        if( bUseThunkLayer )
        {
            if( bRxThunkPending == false )
            {
                PrintLog(1,  "SetUartLocal(): Thunk:  Call SwitchCelFiAddress()" );
                if( bCnxToCu )
                {
                    // Initially connected to CU so make sure we are addressing a CU.
                    Module._SwitchCelFiAddress(CELFI_ADDRESS_ID_CU1);
                }
                else
                {
                    // Initially connected to NU so make sure we are addressing a NU
                    Module._SwitchCelFiAddress(CELFI_ADDRESS_ID_NU);
                }
                
                // Now get the new board config to verify that we are local.
                GetBoardConfig();
            }
            else
            {
                PrintLog(1,  "SetUartLocal(): Thunk called but thunk pending so do nothing" );
            }
        }
        else
        {

            PrintLog(1,  "SetUartLocal(): Super Msg Send: Cancel UART Redirect." );
            ...
            // Modified on 10/10/16 to use the write per LB bug WAVEAPP-24.
            u8TempTxBuff[i++] = NXTY_WRITE_ADDRESS_REQ;             
            u8TempTxBuff[i++] = (NXTY_PCCTRL_UART_REDIRECT >> 24);  
            u8TempTxBuff[i++] = (NXTY_PCCTRL_UART_REDIRECT >> 16);
            u8TempTxBuff[i++] = (NXTY_PCCTRL_UART_REDIRECT >> 8);
            ...
            u8TempTxBuff[i++] = 0x00;
            u8TempTxBuff[i++] = 0x00;
            u8TempTxBuff[i++] = 0x00;                                     // Cancel UART redirect
